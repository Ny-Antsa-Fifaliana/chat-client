...
const Chat = ()=>{
    ...
    const urlDecoded = decodeURIComponent(location.search)
        ...
        useEffect(() => {
            if (messages.length > 0 && messages[messages.length - 1].user) {
                if(messages[messages.length - 1].user.charAt(0).toUpperCase()+messages[messages.length - 1].user.slice(1).toLowerCase()===name.charAt(0).toUpperCase()+name.slice(1).toLowerCase()){
                    notificationSoundReceive.muted=false;
                    playNotificationSoundSend();
                }
                else if(messages[messages.length - 1].user.charAt(0).toUpperCase()+messages[messages.length - 1].user.slice(1).toLowerCase() === 'Admin' && !notificationSoundReceive.muted){
                    playNotificationSoundAdmin();
                }

                else if (notificationSoundReceive.muted){
                    window.addEventListener('click',()=>{notificationSoundReceive.muted=false});
                }
                else{
            ...
            return(
                window.removeEventListener('click',()=>{notificationSoundReceive.muted=false})
            )
        }, [messages, name]);
    ...
    return (
        <Stack className='conteneurChat' direction="column">
            <InfoBar room={room}/>
            <Messages messages={messages} name={name} usersInRoom={usersInRoom}  handleTabChange={handleTabChange} />
            {showInput && <Input message={message} setMessage={setMessage} sendMessage={sendMessage} />}  
        </Stack>
    )   
};
